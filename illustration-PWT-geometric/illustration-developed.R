...
for (i in 1:length(year)) {
  ...
  if (i<length(year)){
    ...
    i1=which(dfa$year==year[i])
    df1a=dfa[i1,]
    x1=cbind(df1a$emp,df1a$cn)
    y1=matrix(df1a$rgdpo,ncol=1)
    
    i2=which(dfa$year==year[i+1])
    df2a=dfa[i2,]
    x2=cbind(df2a$emp,df2a$cn)
    y2=matrix(df2a$rgdpo,ncol=1)
    
    
  } else {
    
    i1=which(dfa$year==year[1])
    df1a=dfa[i1,]
    x1=cbind(df1a$emp,df1a$cn)
    y1=matrix(df1a$rgdpo,ncol=1)
    i2=which(dfa$year==year[i])
    df2a=dfa[i2,]
    x2=cbind(df2a$emp,df2a$cn)
    y2=matrix(df2a$rgdpo,ncol=1)
  ...
  res.agg[i,1:2]=tt$estimate[c(1,3)]
  res.agg[i,3:4]=tt$bounds1[1,]
  res.agg[i,5:6]=tt$bounds1[2,]
  res.agg[i,7:8]=tt$bounds1[3,]
  
  ###############
  ss=coverage.simple.mean(x1=x1,y1=y1,x2=x2,y2=y2)
  
  res.simple[i,1:2]=ss$estimate[c(1,3)]
  res.simple[i,3:4]=ss$bounds1[1,]
  res.simple[i,5:6]=ss$bounds1[2,]
  res.simple[i,7:8]=ss$bounds1[3,]
...
write(tex,file="./Output/coverage-pwt-simple-developed.tex")
...
write(tex,file="./Output/coverage-pwt-agg-developed.tex")
