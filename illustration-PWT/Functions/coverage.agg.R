...
coverage.agg <- function(x1,y1,x2,y2) {
  ...
  mu12=mean(U12)
  #
  xi=-0.5*(log(mu1)-log(mu2)-log(mu3)+log(mu4)+
  ...
  for (j in 1:L) {
    ...
    mu8a=mean(U8a)
    #
    xia=-0.5*(log(mu1a)-log(mu2a)-log(mu3a)+log(mu4a)+
    ...
    mu8b=mean(U8b)
    #
    xib=-0.5*(log(mu1b)-log(mu2b)-log(mu3b)+log(mu4b)+
  ...
  ts=sig/sqrt(n)
  bounds0=matrix((xi-ts*crit),nrow=3,ncol=2)
